...
Crucially, the graph fed to the HGT is materialised from the \emph{training}
interactions only, so neither message passing nor the training objective can
observe held-out edges. Validation and test scores are then computed against
the held-out interactions of the \emph{same} user-level stratified split that
drives the baselines (\cref{sec:eval-protocol}), rather than an independent
random edge split internal to the model. This makes the HGT directly and
fairly comparable to the popularity, KNN-CF and hybrid baselines on identical
folds.
...
The checkpoint with the best validation Recall---measured on the shared
stratified validation ground truth described above---is retained for testing.
