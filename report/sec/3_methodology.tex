...
decoder, trained for 30 epochs with Adam (learning rate $10^{-3}$, batch
size~256) to minimise the mean-squared reconstruction error. The
128-dimensional bottleneck is exported as a dense \emph{content} embedding for
every track and is used both to initialise the \texttt{track} node features of
the graph models and as the item representation of the hybrid baseline. As the
task is pure reconstruction, no test split is required; a 10\% validation
partition is held out only to monitor over-fitting.
...
$[\Re\,\Vert\,\Im]$ after flattening, and the models are trained for 100 epochs
with Adam. The entity embeddings provide a structure-only view of the catalogue
that is used to seed node features and to support the qualitative latent-space
analysis in \cref{sec:results}.
...
\textbf{(iii)~XGBoost two-stage hybrid} first retrieves a pool of candidate
tracks per user via the dot product between a mean autoencoder \emph{user
profile} and the track content embeddings, then re-ranks the pool with a
gradient-boosted learning-to-rank model (\texttt{rank:ndcg}) over the
concatenated $[\text{user-profile}\,\Vert\,\text{track-embedding}]$ features.
The hybrid therefore fuses content (autoencoder) and behaviour (candidate
generation) without using the graph structure, isolating the contribution of
the GNN.
...
popular content. Every model is scored with the same recommendation lists
sliced at $K\in\{5,10,20,50,100\}$, and pairwise differences are assessed with
the paired Wilcoxon signed-rank test (with Cohen's~$d_z$ effect sizes) and an
omnibus Friedman test across all models.
