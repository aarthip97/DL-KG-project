...
Despite sharing the complex-space inductive bias, RotatE is expected to be stronger here because it additionally models \emph{compositional} relation patterns, which arise naturally in our graph (e.g.\ the chain \textit{track~$\xrightarrow{\text{hasTempoClass}}$~Allegro~$\xrightarrow{\text{followedBy}}$~decade} or genre-hierarchy traversals). A rotation composed with another rotation is still a rotation, so RotatE represents such chains geometrically, whereas ComplEx has no analogous closure property. RotatE was therefore used as the primary initialiser for the HGT node features, with ComplEx retained for comparison.
...
The primary recommendation model was a Heterogeneous Graph Transformer (HGT)~\cite{hu2020hgt}, chosen for its ability to operate natively on graphs with multiple node and edge types---essential for our KG of tracks, artists, users, genres, instruments, keys, modes, tempos, and decades. Unlike standard GNNs that aggregate over neighbours without modelling type differences, HGT learns type-specific transformation and attention parameters for each node- and edge-type combination, preserving the graph's heterogeneous structure throughout message passing and capturing more nuanced relationships for recommendation.
...
($lr = 10^{-3}$, weight decay $=10^{-4}$) with a single-cycle
cosine-annealing schedule that decays the learning rate smoothly to
near zero over the full training run. Model selection and early
stopping were both driven by the composite validation \emph{Overall
Score} at $K{=}10$ (Eq.~\ref{eq:overall}) rather than a single accuracy
metric: the checkpoint maximising this score was retained for final
evaluation, so that selection rewards ranking quality, catalogue
coverage, and popularity debiasing jointly.
...
All recommenders were evaluated on a single, shared \emph{user-level
stratified} split of the interaction data (train/validation/test),
stratified by song-level attributes (genre and release decade) so that
popularity and content distributions are preserved across folds. Song
features and graph structure are visible to every split and only the
interactions are partitioned, preventing leakage. The unsupervised KG
embeddings and autoencoder are fit on the training split only.

For an evaluated user $u$ with held-out ground-truth set $\mathcal{G}_u$
and ranked top-$K$ list $R_u=(r_1,\dots,r_K)$, let
$g_i=\mathbf{1}[r_i\in\mathcal{G}_u]$ indicate whether the $i$-th
recommendation is relevant. We report a standard top-$K$ suite, each
metric averaged over evaluated users: precision
$\mathrm{P@}K=\tfrac{1}{K}\sum_{i\le K} g_i$; recall
$\mathrm{R@}K=\bigl(\sum_{i\le K} g_i\bigr)/\min(K,|\mathcal{G}_u|)$;
hit-rate $\mathrm{HR@}K=\mathbf{1}\!\left[\sum_{i\le K} g_i>0\right]$;
and the reciprocal rank $\mathrm{MRR}=1/\min\{i:g_i{=}1\}$ (zero when no
recommendation is relevant). Ranking quality is the normalised
discounted cumulative gain
$\mathrm{NDCG@}K=\mathrm{DCG@}K/\mathrm{IDCG@}K$, with
$\mathrm{DCG@}K=\sum_{i\le K} g_i/\log_2(i+1)$ and $\mathrm{IDCG@}K$ the
DCG of the ideal ranking. Two catalogue-level metrics capture diversity:
coverage $\mathrm{Cov}=\bigl|\bigcup_u R_u\bigr|/|\mathcal{I}|$, the
fraction of the catalogue $\mathcal{I}$ ever recommended, and mean
popularity-bias $\mathrm{PB@}K=\tfrac{1}{K}\sum_{i\le K}\pi(r_i)$, the
average normalised popularity $\pi(\cdot)\in[0,1]$ of the recommended
items. These are fused into a single composite \emph{Overall Score}
\begin{equation}\label{eq:overall}
\mathrm{Overall@}K = 0.6\,\mathrm{NDCG@}K + 0.2\,\mathrm{Cov}
+ 0.2\,\bigl(1-\mathrm{PB@}K\bigr),
\end{equation}
which rewards models that are accurate \emph{and} surface diverse, less
popular content; it is also the criterion used to select the HGT
checkpoint during training. Every model was scored on the same lists at
$K\in\{5,10,20,50,100\}$, and pairwise differences were assessed with the
paired Wilcoxon signed-rank test (Cohen's~$d_z$ effect sizes) and an
omnibus Friedman test on a shared random subsample of 5{,}000 users,
since full inference over all 284{,}864 test users is computationally
prohibitive for the XGBoost model.
