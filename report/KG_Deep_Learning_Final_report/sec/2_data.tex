...
After the data integration and filtering process, the subsequent step included feature extraction and selection. Since raw MIDI files do not directly provide numerical descriptors suitable for machine learning, each track had to be converted into a fixed-length feature vector capturing its musical characteristics. For this purpose the jSymbolic \cite {mckay2018jsymbolic} and music21 \cite {music21docs}feature extractors were used. They work directly with symbolic representations of music and generate a wide range of high-level descriptors based on key concepts of music theory, making it particularly useful for extracting meaningful and interpretable features from MIDI data.

Because of the substantial memory footprint, the jSymbolic extraction was implemented using batches of 100 songs at a time,  which produced  XML files containing the computed feature values together with a feature-definition schema. Each resulting XML output was subsequently converted into a tabular CSV representation, and each row was associated with its corresponding song\_id by matching the MIDI file hash against the metadata table assembled in the previous step. Out of the 19,037 matched tracks, 7,013 were successfully processed constituting the basis for all downstream analysis and modeling. The music21 native feature extractors were also run in batches of 100 songs at a time, producing CSV files which were later concatenated into one complete file containing all the music21 feature data. These were then combined into one dataframe along with all the metadata for each song.
